...
class Board:
    ...
    def max_tile(self):
        return 2 ** np.max([cell for row in self.unpack_rows(self.state) for cell in self.unpack_cells(row)])
    ...
    def state_as_grid(self):
        rows = self.unpack_rows(self.state)
        grid = np.stack([self.unpack_cells(row) for row in rows])
