...
class Agent:
    ...
    def _init(self, ckpt_dir):
        ...
        memory = PrioritizedExperienceReplay(100, ckpt_dir)
    ...
    def train(self, args):
        dqn_online, dqn_target, optimizer, memory, ckpt, ckpt_manager = self._init(args.ckpt_dir)
        policy = EpsilonGreedyPolicy(self.env, dqn_online)
        summary_writer = tf.summary.create_file_writer(args.log_dir)

        with tqdm(total=args.max_action_steps, desc="Training", unit="action") as pbar:
            pbar.update(ckpt.action_step.numpy())
            observation = self.env.reset()
            episode_reward, episode_score = 0, 0
            for i in range(ckpt.action_step.numpy(), args.max_action_steps + 1):
                state = self.env.preprocess(observation)
                action = policy.take_action(state, ckpt.action_step)
                ...
                if i % args.train_steps == 0:
                    sample_indices, samples, importance_sampling_weights = memory.sample(args.batch_size)
                    ...
                    loss, td_errors = self._train_step(states, actions, rewards, next_states, dones,
                                                       ...
                                                       dqn_online, dqn_target, optimizer, args.gamma)
                ...
                if i % args.update_dqn_target_steps == 0:
                    dqn_target.set_weights(dqn_online.get_weights())
